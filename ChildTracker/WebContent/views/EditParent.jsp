...
<%@ page import = "database.*" import = "beans.*" %>
<!DOCTYPE html>
<html>
<head>
...
<body>
	...
	<%
		...
		Parent p = userDAO.getParentById(Integer.valueOf(parentId));
		
	%>

	<div class="container register">
		<h3 style="text-align: center">Basic Information</h3>
		<form method="post" action="../ProcessEditParent">
			<div class="row">
				<div class="col-md-6">
					<div>
						First Name* <input type="text" class="form-control" value="<%=p.getFirstName() %>"
							name="firstName" />
					</div>
					<div>
						Last Name* <input type="text" class="form-control" value="<%=p.getLastName() %>"
							name="lastName" />
					</div>
					<div>
						Password* <input type="password" class="form-control" value="<%=p.getPassword() %>"
							name="password" />
					</div>
					<div>
						Email* <input type="email" name="email" class="form-control" value="<%=p.getEmail()%>">
					</div>
				</div>
				<div class="col-md-6">
					<div>
						Phone* <input type="tel" class="form-control" value="<%=p.getPhone() %>"
							 name="phone">
					</div>
					<div>
						Address * <input type="text" class="form-control" value="<%=p.getAddress() %>"
							name="address" />
					</div>

					<div>
						Date of Birth* <input type="date" class="form-control"
							placeholder="Date of Birth *" value="<%=p.getBirthdate() %>" name="birthdate" />
					</div>
			...
			<div class="row">
				<div class="col-md-4">
					<button name="register" value="Register">Save</button>
				</div>
